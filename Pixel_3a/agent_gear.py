...
def get_reward(fps, target_fps, super_big_clock, big_clock, little_clock, gpu_clock, soc_temperature):
    reward = (fps - target_fps) * 20 +  (super_big_clock / int(cpu_freq_list[-1][-1]) + big_clock / int(cpu_freq_list[1][-1]) + little_clock / int(cpu_freq_list[0][-1]) + gpu_clock / int(gpu_freq_list[-1])) * -100 - soc_temperature / 20 
    return reward
    ...
    try:
        while t < experiment_time:
            ...
            t += 1
             ...
             # 下面这段是记录进文件，供绘图使用的 
            f.write(f'{t},{underlying_data[0]},{underlying_data[1]},{underlying_data[2]},{underlying_data[3]},{underlying_data[4]},{fps}\n')
            f.flush() 
            print('[{}] state:{} action:{} next_state:{} reward:{} fps:{}'.format(t, state,action,next_state,reward,fps))

            if t % 100 == 0:
                agent.save_model(t, 'save_model/model.pth')
