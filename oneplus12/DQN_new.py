...
class DQNAgent:
    def __init__(self, input_dim, output_dim, learning_rate=0.001, gamma=0.02, buffer_size=500000, batch_size=64):
        ...
        self.model = load_model('dqn_model_episode_10000')
    ...
    def train(self, env, episodes=1000, epsilon_start=1.0, epsilon_end=0.01, epsilon_decay=0.9999):
        epsilon = epsilon_start
        state = env.reset()

        for episode in range(episodes):
            action = self.select_action(state, epsilon)
            next_state, reward = env.step(action)  # 移除done
...
env = Environment()
dqn = DQNAgent(10, 625)
dqn.train(env, episodes=10001, epsilon_start=0.99, epsilon_end=0.02)
